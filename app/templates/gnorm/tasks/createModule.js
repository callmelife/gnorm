...
gulp.task('create-module', function() {
  ...
  gulp.src('./app/scripts/modules/index.js').pipe(modify({
    fileModifier: function(file, contents) {
      ...
      if (modulesArrSanitized[0] === '') {
        ...
        let err = new gutil.PluginError('template', 'The module "' + argv.name + '" already exists. Please choose a unique module name.');
        throw err;
      }
      ...
      return content.replace(/(modules+[:\s]*{)([\w\s\r\t\/:()'".,]*)*(})/g, '$1' + modulesArrSanitized + '$3');
    }
  }))

  // Beautify the file
    .pipe(prettify({config: '.jsbeautifyrc', mode: 'VERIFY_AND_WRITE'})).pipe(gulp.dest('./app/scripts/modules'));

  // Scaffold the template files into the module folder
  gulp.src('./gnorm/templates/loader.js').pipe(rename(argv.name + '.load.js')).pipe(template(argv)).pipe(gulp.dest('./app/scripts/modules/' + argv.name));
  gulp.src('./gnorm/templates/module.js').pipe(rename(argv.name + '.main.js')).pipe(template(argv)).pipe(gulp.dest('./app/scripts/modules/' + argv.name));
